...
public class RTSCameraController : MonoBehaviour {
    ...
    public bool doEdgePanning = false;
	...
	void Update () {
        ...
        #region Movement
            ...
            if (Input.GetKey(KeyCode.W) || (doEdgePanning && Input.mousePosition.y >= Screen.height - ScreenEdgeBorderThickness))
            {
                panMovement += Vector3.forward * panSpeed * Time.deltaTime;
            }
            if (Input.GetKey(KeyCode.S) || (doEdgePanning && Input.mousePosition.y <= ScreenEdgeBorderThickness))
            {
                panMovement -= Vector3.forward * panSpeed * Time.deltaTime;
            }
            if (Input.GetKey(KeyCode.A) || (doEdgePanning && Input.mousePosition.x <= ScreenEdgeBorderThickness))
            {
                panMovement += Vector3.left * panSpeed * Time.deltaTime;
            }
            if (Input.GetKey(KeyCode.D) || (doEdgePanning && Input.mousePosition.x >= Screen.width - ScreenEdgeBorderThickness))
            {
